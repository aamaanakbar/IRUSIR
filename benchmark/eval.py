...
DATASET_NAME = "FFHQ-X"
DATASET_PATH = "/home/akbar/Documents/All_codes/robust-unsupervised/datasets/samples_resized"
...
def make_crops(
    ...
    def process_image(i, im_path):
        ...
        for k in range(num_crops_per_image):
            save_image(cropping(pred), f"{out_path}/{i:04d}/{k:04}.JPG")
...
def crop_dataset():
    if True:
        make_crops(
            globr(f"{DATASET_PATH}/{DATASET_NAME}/**/*.JPG"),
...
def eval_experiment(
    ...
    for suffix in suffixes:
        if COMPUTE_FID := True:
            make_crops(
                globr(f"{expr_path}/**/pred{suffix}.JPG"),
        ...
        if COMPUTE_DISTANCE_METRICS := True:
            ...
            for im_path in globr(f"{expr_path}/inversions/**/pred{suffix}.JPG"):
                def imopen(x):
